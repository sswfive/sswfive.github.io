...
#### 删除：
```python
del tmplist[index]   # 删除指定索引的数据（从内存中删除）
...
- tuple 元组，是一个可以放置任意数据类型的有序集合
- tuple是静态的（不可变对象），长度大小固定，无法对元素进行增删改
...
len(tmptuple)  # 4  统计元组中数据的个数

tmptuple2 = (10, 20, ['aa', 'bb', 'cc'], 50, 30)
print(tmptuple[2])  # 访问到列表
